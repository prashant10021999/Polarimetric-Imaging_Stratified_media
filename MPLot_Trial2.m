...
% Step 6: Create a placeholder X-axis based on K-space grid
kx_grid = linspace(-maxAOI, maxAOI, Npts);  % X-axis placeholder (K-space angles)

% Step 7: Create a placeholder for mmError (zeros of the same size as MM)
mmError = zeros(size(MM));  % Required to avoid input error

% Step 8: Plot only the first element (M11)
figure;
pObj = MPlot(kx_grid, MM);
pObj.plot(kx_grid, squeeze(MM(1,1,:,:)), squeeze(mmError(1,1,:,:)), '-');
pObj.print('MuellerMatrixPlot_M11.eps');
